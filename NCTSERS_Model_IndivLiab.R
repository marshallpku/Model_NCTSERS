# This script calculates individdual liabilities and normal costs for NCTSERS members. 
...
liab.active %<>%   
  mutate(reduction2 = min(0.05 * (30 - yos),
                          ifelse(age < 60, (60 - age) * 0.05 + 0.15, 
                                 ifelse(age < 65, (65 - age) * 0.03, 0 ))),
...
liab.la %<>% as.data.frame  %>% 
  ...
  select(year, ea, age, year.r, age.r, start.year, B.la, ALx.la, sx, ax.r.W.ret, benefit, Bx.laca, COLA.scale) %>% 
