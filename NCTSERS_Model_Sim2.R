...
run_sim <- function(Tier_select_,
  ...
  # i is assumed interest rate. LGs of each period will be amortized seperately.  
  # SC: Supplement cost 
  ...
  penSim0$AL.la    <- AggLiab_$la.current.new[, "ALx.la.sum"]    + AggLiab_$la.entrants[, "ALx.la.sum"]
  penSim0$AL.ca    <- AggLiab_$ca.current.new[,   "liab.ca.sum"] + AggLiab_$ca.entrants[, "liab.ca.sum"]
  penSim0$AL.term  <- AggLiab_$term.current.new[, "ALx.v.sum"]  +  AggLiab_$term.entrants[, "ALx.v.sum"]
  penSim0$AL.death <- AggLiab_$death[,"ALx.death.sum"]
  penSim0$AL.disb.la  <- AggLiab_$disb.la.current.new[, "ALx.disb.la.sum"] + AggLiab_$disb.la.entrants[, "ALx.disb.la.sum"]
  penSim0$AL.disb.ca  <- AggLiab_$disb.ca[, "ALx.disb.ca.sum"]
  penSim0$AL.initBen  <- AggLiab_$initBen_calib[, "ALx.initBen.calib"]
  
  penSim0$AL       <- with(penSim0, AL.act + AL.la + AL.ca +  AL.term + AL.death + AL.disb.la + AL.disb.ca + AL.initBen)
  }
  ...
  # PVFS(j)
  penSim0$PVFS <- AggLiab_$active[, "PVFSx.sum"] 
  ...
  if(!calibration){
    ...
    penSim0$B.la    <- AggLiab_$la.current.new[, "B.la.sum"] + AggLiab_$la.entrants[, "B.la.sum"]
    penSim0$B.ca    <- AggLiab_$ca.current.new[, "B.ca.sum"] + AggLiab_$ca.entrants[, "B.ca.sum"]
    penSim0$B.v     <- AggLiab_$term.current.new[, "B.v.sum"] +  AggLiab_$term.entrants[, "B.v.sum"]
    penSim0$B.death <- AggLiab_$death[, "B.death.sum"]
    penSim0$B.disb.la  <- AggLiab_$disb.la.current.new[, "B.disb.la.sum"] + AggLiab_$disb.la.entrants[, "B.disb.la.sum"]
    penSim0$B.disb.ca  <- AggLiab_$disb.ca[, "B.disb.ca.sum"]
    penSim0$B.initBen  <- AggLiab_$initBen_calib[, "B.initBen.calib"] 
    penSim0$B       <- with(penSim0, B.la + B.ca + B.v + B.death + B.disb.la + B.disb.ca + B.initBen)
  }
  ...
  penSim_results <- foreach(k = -1:nsim, .packages = c("dplyr", "tidyr")) %dopar% {
    ...
    for (j in 1:nyear){
      ...
      if(j == 1 & k!=-1) penSim$ERC[j] <- max(min(penSim$ERCrate_max[j] * penSim$PR[j], penSim$ERC[j]), (0.1113 + 0.0035) *  penSim$PR[j]) 
      
      if(j %in% 2:ECRSP_year & k!=-1) penSim$ERC[j] <- max(min(penSim$ERCrate_max[j] * penSim$PR[j], penSim$ERC[j]),  (penSim$ERC[j-1] /penSim$PR[j-1] + 0.0035) *  penSim$PR[j])
      }
