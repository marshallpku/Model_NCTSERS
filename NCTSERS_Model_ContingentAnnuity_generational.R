# This script models the contingent annuitants (all retirees) in NC TSERS
...
get_contingentAnnuity <- function(Tier_select_,
  ...
  # factor.ca = paramlist$factor.ca
  # range_age.ben = min(paramlist$range_age):100
  # apply_reduction = T
  # decrement.model_ = decrement.model
  # paramlist_        = paramlist
  # Global_paramlist_ = Global_paramlist
...
get_liab.ben <- function(gender.R, 
  ...
  # Extend the mortality table to 3 years below the lowest retirement age for each start.year group
  mortality <-
  ...
  df %<>% mutate(year.r_age.r = paste0(year.r, age.r)) # for easier splitting
  
  df <- split(df, df$year.r_age.r) %>% lapply(fn_demo) %>% bind_rows # %>% select(-year.r_age.r)
  
  
  # split(df, list(df$year.r, df$) )
  
  # # check demographic
  #df.check <- df1 %>% mutate(tot.check = n.R1S1 + n.R1S0 + n.R0) %>% select(age, age.S, n.R1S1, n.R1S0, n.R0S1, tot.check) 
  #df.check # Look good!
  
  ## Aggregate liability and benefit payment  ##
  
  df %<>% select(year.r, age.r, age, age.S, B.la, liab.ret.indiv.la, liab.ret.indiv.ca, liab.ben.indiv,
                 n.R1S1, n.R1S0, n.R0S1, year.r_age.r) %>% 
    group_by(year.r, age.r) %>% 
    mutate(
...
# Check internal consistency (optional)
 ...
 fn_MA <- function(df){
  ...
  # df <- split(df, df$year.r_age.r) %>% lapply(fn_MA) %>% bind_rows %>% 
...
reduction.M <- get_liab.ben(gender.R = "M") %>% 
               group_by(year.r, age.r) %>% filter(age == min(age)) %>% select(year.r, age.r, reduction) 


# For female retirees
# w/o benefit reduction  save the benefit reductin factor for contingent annuity. 
reduction.F <- get_liab.ben(gender.R = "F") %>% 
               group_by(age.r) %>% filter(age == min(age)) %>% select(year.r, age.r, reduction) 
} else {
  reduction.M <- 1
  reduction.F <- 1
}
...
df.all <-   select(df.M, year.r, age.r, age, 
  ...
  left_join(select(df.F, year.r, age.r, age, 
...
liab.ca <- df.all %>% select(year.r, age.r, age, year, 
                             liab.ca.sum.1, liab.R1S1.sum.1, liab.R1S0.sum.1, liab.R0S1.sum.1,
