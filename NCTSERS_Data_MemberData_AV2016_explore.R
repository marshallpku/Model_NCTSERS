...
file_memberData <- "Data_inputs/NCTSERS_MemberData_AV2016.xlsx"
...
# Import actives
import_actives(file_memberData, "Actives", "Actives")

# data structure of outputs
  # $agecuts:
  #   - age.cell
  #   - agelb
  #   - ageub
...
init_actives_all %>% 
  filter(age %in% 20:34, yos %in% 0:14) %>% 
  select(age, yos, nactives) %>% 
  spread(yos, nactives)

init_actives_all %>% 
  filter(age %in% 20:54, yos %in% 0:19) %>% 
...
save(init_actives_all, 
     ...
     init_terms_all,
     occupGender,
     file = "Data_inputs/NCTSERS_MemberData_AV2016.RData")
