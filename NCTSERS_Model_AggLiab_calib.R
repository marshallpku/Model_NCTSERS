...
get_AggLiab <- function( Tier_select_,
  ...
  # Notes on naming conventions:
   ...
   # "sum" denotes sum of AL/NA/... across age * ea in each year. 
   # "av"  denotes sum of variables related to life annuity, contingent annuity, and term benefits for actives. (now plus death benefit and disability benefit)
  ...
  la.agg.current.init <- liab_$la %>% 
    ...
    summarise(ALx.la.sum      = sum(ALx.la.tot, na.rm = TRUE),
              ...
              nla             = sum(number.la , na.rm = TRUE)) %>% 
    # mutate(runname = runname) %>% 
    right_join(data.frame(year = init.year:(init.year + nyear-1))) %>% 
  ...
  death.agg <- data.frame(year = init.year:(init.year + nyear - 1)) %>% 
    mutate(ALx.death.sum = 0,
           B.death.sum   = 0,
           ndeathBen     = 0) %>% 
    as.matrix
  ...
  # current initial
  disb.la.agg.current.init <- liab_$disb.la %>% 
    filter(year - (age-ea) <= init.year, year.disb <= init.year) %>%
    group_by(year) %>% 
    summarise(ALx.disb.la.sum   = sum(ALx.disb.la.tot, na.rm = TRUE),
              B.disb.la.sum     = sum(B.disb.la.tot  , na.rm = TRUE),
              ndisb.la          = sum(number.disb.la , na.rm = TRUE)) %>% 
  ...
  # current new
  disb.la.agg.current.new <- liab_$disb.la %>% 
    filter(year - (age-ea) <= init.year, year.disb > init.year) %>%
    group_by(year) %>% 
    summarise(ALx.disb.la.sum   = sum(ALx.disb.la.tot, na.rm = TRUE),
              B.disb.la.sum     = sum(B.disb.la.tot  , na.rm = TRUE),
  ...
  # entrants
  disb.la.agg.entrants <- liab_$disb.la %>%
    filter(year - (age-ea) > init.year) %>%
    group_by(year) %>% 
    summarise(ALx.disb.la.sum   = sum(ALx.disb.la.tot, na.rm = TRUE),
              B.disb.la.sum     = sum(B.disb.la.tot  , na.rm = TRUE),
  ...
  disb.la.agg 
  disb.la.agg.current.init
  disb.la.agg.current.new 
  disb.la.agg.entrants
  ...
  # current init
  term.agg.current.init <- liab_$term %>% 
    filter(year - (age-ea) <= init.year, year.term <= init.year) %>%
    group_by(year) %>% 
    summarise(ALx.v.sum   = sum(ALx.v.tot, na.rm = TRUE),
              B.v.sum     = sum(B.v.tot  , na.rm = TRUE),
              nterms      = sum(number.v , na.rm = TRUE)) %>% 
  ...
  # current new
  term.agg.current.new <- liab_$term %>% 
    filter(year - (age-ea) <= init.year, year.term > init.year) %>%
    group_by(year) %>% 
    summarise(ALx.v.sum   = sum(ALx.v.tot, na.rm = TRUE),
              B.v.sum     = sum(B.v.tot  , na.rm = TRUE),
  ...
  term.agg
  term.agg.current.init 
  term.agg.current.new
  term.agg.entrants 
  ...
  init.ca.agg <- expand.grid(init.age = unique(init_beneficiaries_$init.age), age = min(init_beneficiaries_$init.age):max.age) %>%
                 ...
                 mutate(age.r = age.r[age == init.age], # necessary?
                        year  =  init.year + age -  init.age) %>% 
                 left_join(select(mortality.post.model_, year, age, age.r, ax.r.W.ben = ax.r.W, pxm.post.W)) %>% 
                 mutate(
  ...
  init.ret.ca.agg <- expand.grid(age.r = unique(init_retirees.ca_$age.r), age = min(init_retirees.ca_$age.r):max.age) %>%
    ...
    mutate(year =  init.year + age - age.r) %>% # year is need to join generational liab.ca
    left_join(liab.ca_) %>% 
    group_by(age.r) %>% 
    arrange(age.r, age) %>% 
    # mutate(age.r = age.r[age == init.age]) %>% # necessary?
  ...
  new.ca.agg <- expand.grid(year.r = init.year:(init.year + nyear - 1), age.r = range_age.r, ea = range_ea, age = range_age) %>% 
   ...
   ca.agg.current.init <-   
     ...
     left_join(init.ret.ca.agg) %>% 
     colwise(na2zero)() %>% 
     mutate(n.R1     = n.ret.ca.R1.init,
            n.R0S1   = n.R0S1.init + n.ret.ca.R0S1.init,
            B.ca.sum = B.ca.sum.init + B.ret.ca.sum.init,
            liab.ca.sum = liab.ca.sum.init + liab.ret.ca.sum.init) %>% 
     right_join(data.frame(year = init.year:(init.year + nyear-1))) %>% 
     mutate_all(funs(na2zero)) %>%
     as.matrix
   ...
   ca.agg.current.new <- 
     expand.grid(year.r = init.year:(init.year + nyear - 1), age.r = range_age.r, ea = range_ea, age = range_age) %>% 
     mutate(year = year.r + age - age.r) %>% 
     filter(age >= ea,
            age >= age.r,
            age.r > ea,
  ...
  AggLiab <-  list(active = active.agg, 
                   ...
                   ca     = ca.agg, 
                   ca.current.init = ca.agg.current.init,
                   ca.current.new  = ca.agg.current.new,
                   ca.entrants     = ca.agg.entrants,
                   
                   term  = term.agg,
                   term.current.init = term.agg.current.init,
                   term.current.new  = term.agg.current.new,
                   term.entrants     = term.agg.entrants,
                   
                   disb.la = disb.la.agg,
                   disb.la.current.init = disb.la.agg.current.init,
                   disb.la.current.new  = disb.la.agg.current.new,
                   disb.la.entrants     = disb.la.agg.entrants,
